...
if ( class_exists( 'WooCommerce' ) ) {
    ...
    // Remove default result count and ordering (custom template handles these)
...
function shopco_customizer( $wp_customize ) {
    ...
    $wp_customize->add_section( 'shopco_dress_style', array(
        'title'    => __( 'Browse by Dress Style', 'shopco' ),
        'priority' => 33,
    ) );
    ...
    foreach ( $dress_styles as $slug => $label ) {
        $wp_customize->add_setting( 'shopco_style_image_' . $slug, array(
            'default'           => '',
            'sanitize_callback' => 'esc_url_raw',
        ) );

        $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'shopco_style_image_' . $slug, array(
            'label'   => sprintf( __( '%s Style Image', 'shopco' ), $label ),
            'section' => 'shopco_dress_style',
        ) ) );
    }
